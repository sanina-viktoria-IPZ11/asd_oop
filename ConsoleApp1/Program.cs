       ...
       static void CratingOfArray(ref int[] ArrayOne, ref int size)
        ...
        static int BinSearch(ref int[] ArraySortOne, ref int size)
        {
            ...
            else
            {
                ...
                //початок виміру часу
                stopWatch.Start();
                while ((leftBarrier < rightBarrie))
                {
                    middle = (rightBarrie + leftBarrier) / 2; ;
                    if (ArraySortOne[middle] < root) leftBarrier = middle + 1;
                    else if (ArraySortOne[middle] > root) rightBarrie = middle - 1;
                    else  {result = middle; b = 1; }
                }
                if (b == 1) Console.WriteLine("Шуканий елемент на позиції: " + result);
